...
// import { qaMentorPrompts } from '../config/qaMentorPrompts' // 미사용이므로 제거
...
interface ChatMessage {
  ...
  export async function getChatResponse(
    category: string,
    _message: string, // underscore prefix로 미사용 표시
    _previousMessages: ChatMessage[] = [] // underscore prefix로 미사용 표시
  ): Promise<ChatResponse> {
    // 실제로는 백엔드 API 호출
    // const response = await fetch('/api/qa-mentor', {
    //   method: 'POST',
    //   headers: { 'Content-Type': 'application/json' },
    //   body: JSON.stringify({ category, message: _message, previousMessages: _previousMessages })
    // })
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        resolve({
          response: generateMockResponse(category),
          category,
  ...
  // 목업 응답 생성
  function generateMockResponse(category: string): string {
