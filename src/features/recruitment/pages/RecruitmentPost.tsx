// src/features/recruitment/pages/RecruitmentPost.tsx
...
const RecruitmentPost = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. 결제가 필요한 경우 (무료가 아닌 경우)
      if (formData.packageType !== 'basic' || true) { // 실제로는 무료 조건 체크
        // 결제 초기화
        ...
        await PaymentService.processPayment(
          ...
          user?.name || formData.contactName
        );
        
        // 결제 페이지로 이동하므로 여기서 종료
        return;
      }
      
      // 3. 무료인 경우 완료 페이지로 이동
      navigate('/services/recruitment/post/complete', {
        state: { jobId, email: formData.contactEmail, managementToken }
      });
